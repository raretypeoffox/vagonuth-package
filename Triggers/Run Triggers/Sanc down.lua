...
function SancSelfAtLord()
  if IsClass({"Berserker", "Sorcerer", "Necromancer"}) then return end
...
if SafeArea() then return end
if IsClass({"Necromancer"}) then return end
...
if StatTable.Level == 125 then
  ...
  if IsNotClass({"Priest"}) then SancSelfAtLord(); return end
  ...
  if IsClass({"Monk", "Shadowfist"}) and StatTable.Level >= 30 then CastAfterCombat("'iron monk'"); return end

  -- Cleric, Priest, Druid, and Vizier can use sanctuary at low mort
  if IsClass({"Cleric", "Priest", "Druid", "Vizier"}) and StatTable.Level >= 23 then CastAfterCombat("sanctuary"); return end

  -- Everyone else gets sanc at Hero 45
  if IsNotClass({"Berserker", "Necromancer"}) and StatTable.SubLevel >= 45 then CastAfterCombat("sanctuary"); return end
