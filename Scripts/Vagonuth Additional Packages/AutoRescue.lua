...
function AR.Debug(message)
  --return message -- comment out when debugging
...
function AR.ProcessRescueStack(InternalFunctionLock)
  ...
  if InternalFunctionLock then return end -- return if function is already in recursive loop
  ...
  send("rescue " .. AR.RescueStack[1].name, false)
...
function AR.Show()
  ...
  for name,listed in pairs(AR.RescueList) do
    if listed == true then
      ...
      count = count + 1
      if count % 10 == 0 then -- gtell has a line maximum, after 10 names, start a new line
        list = list .. "|N|;gtell |BR|"
      else
        list = list .. " |BW|||BR| "
      end
    end
