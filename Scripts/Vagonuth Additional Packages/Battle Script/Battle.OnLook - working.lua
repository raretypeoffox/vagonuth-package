-- Script: Battle.OnLook - working
-- Battle.OnLook - working() called on the following events:
-- gmcp.Room.Players
-- gmcp.Room.AddPlayer
...
function Battle.OnLook()
  ...
  for k,v in pairs(Players) do
    ...
    if (tonumber(Players[k].name) ~= nil) then 
        ...
        if StatTable.Race == "Illithid" then Battle.ScrambleCheck(Players[k]) end
...
function Battle.ImmoCheck(mobname, mobnum)
  ...
  local cmd_to_send = nil
  
  if not StatTable.Immolation then
    cmd_to_send = "cast immolation " .. mobnum
  elseif not StatTable.AstralPrison and StatTable.Level == 125 then
    cmd_to_send = "cast 'astral prison' " .. mobnum
  end
  
  if not cmd_to_send or cmd_to_send == "" then return end

  local arc_mod = (Battle.GetSpellCostMod and Battle.GetSpellCostMod("arcane")) or 1
  if StatTable.current_mana > (70000 * arc_mod) then
    cmd_to_send = "quicken 9" .. cs .. cmd_to_send .. cs .. "quicken off"
  elseif StatTable.current_mana > (50000 * arc_mod) then
    cmd_to_send = "quicken 5" .. cs .. cmd_to_send .. cs .. "quicken off"
  end
  
  TryAction(cmd_to_send, 5)
...
-- Returns true if mob has a caster spec
function Battle.IsCaster(mob)
  if not mob or not mob.spec then return false end
  return ArrayHasValue(CasterSpecs, mob.spec)
end
...
function Battle.PsyphonCheck(mob)
  if StatTable.PsyphonExhaust or
     ...
     not Battle.IsCaster(mob) then
    return
  end

  local i = string.find(s, "is here, fighting")
...
function Battle.ScrambleCheck(mob)
  if StatTable.RacialScrambleFatigue or
     not Battle.Combat or
     not Battle.IsCaster(mob) then
    return
  end
  ...
  TryFunction("ScrambleAct", Battle.NextAct, {"racial scramble " .. mob.name, 5}, 5)
end