    ...
    elseif MyClass == "Black Circle Initiate" then
      if MyLevel == 51 then
        if StatTable.SubLevel < 101 then
          ...
          GlobalVar.KillStyle = "shadowcast overconfidence"
        end
...
function SaveProfileVars()
  ...
  for _, pair in ipairs(PROFILE_VARIABLES) do
    ...
    -- Check if the variable exists in GlobalVar, regardless of its value
...
function LoadProfileVars()
  ...
  -- Load all variables defined in PROFILE_VARIABLES
  for _, pair in ipairs(PROFILE_VARIABLES) do
    local varName, defaultValue = pair[1], pair[2]
    -- Check if the variable exists in GlobalVar.Saved, regardless of its value
